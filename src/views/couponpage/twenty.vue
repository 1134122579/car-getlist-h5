...
export default {
  ...
  methods: {
    ...
    // 获取列表
    getCouponLog() {
      this.Api.getCouponLog1({ status: this.status }).then(res => {
    ...
    // 领取
    ReceiveCoupon() {
      this.Api.ReceiveCoupon1().then(res => {
    ...
    iscodebutton(coupon_code, item) {
      ...
      let text = `http://api.skyorange.cn/wxh5/applypagetwenty?couponcode=${coupon_code}`
    ...
    qrcode() {
      ...
      let text = `${location.origin}/applypage?couponcode=0`
...
.index-container {
  ...
  .button_ {
    ...
    .coup_bg {
      background: url('../../assets/coupon_bg20.jpg') no-repeat;
      background-size: 100% auto;
      height: 6.13333rem;
